...
public abstract class ArrayList<T> implements ListADT<T>, Iterable<T>
{
    ...
    {
        T result;
        result = list[index];// result will equal the actual value
       ((Athlete)result).setSpeed(((Athlete)element).getSpeed());
    ...
    {
    	String result=null;
    	result=((Athlete)list[index]).getName();
    	return result;
    }
    ...
    public int find(T target)
    {
        ...
        if (!isEmpty())
            while (result == NOT_FOUND && scan < rear)
                if (((Athlete)target).getName().compareTo(((Athlete)list[scan]).getName())==0)
                    result = scan;
                else
                    scan++;
